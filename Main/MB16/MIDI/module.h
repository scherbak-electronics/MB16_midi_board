...
#define MIDI_EventsProcess() {\
    ...
    if (MIDI_In_isNewMessage()) {\
        ...
        if (MIDI_STATUS_MASK_CTRL_CHANGE_MSG == MIDI_getStatusByte(midi.in.currentStatus)) {\
            App_MIDI_In_ControlChangeEvent(midi.in.msgBuffer[midi.in.msgBufferReadPointer].data1, midi.in.msgBuffer[midi.in.msgBufferReadPointer].data2);\
        }\
